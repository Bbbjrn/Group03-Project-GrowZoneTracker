...
fetch(
  `https://perenual.com/api/species-list?key=sk-YLTu6685827296e1c6133`,
  requestOptions
)
...
fetch(
  `https://perenual.com/api/species-list?key=sk-YLTu6685827296e1c6133&hardiness=${hardinessInput}`,
  requestOptions
)
  .then((response) => response.json())
  .then((result) => console.log(result))
...
const createSpeciesCard = (species) => {
  ...
  img.src =
    species.default_image && species.default_image.medium_url
  ...
  cardText.innerHTML = `
        <strong>Scientific Name:</strong> <em>${
          species.scientific_name.length > 0
            ? species.scientific_name.join(", ")
            : "N/A"
        }</em> <br>
        <strong>Other Names:</strong> ${
          species.other_name.length > 0 ? species.other_name.join(", ") : "N/A"
        } <br>
        <strong>Cycle:</strong> ${species.cycle || "N/A"} <br>
        <strong>Sunlight:</strong> ${
          species.sunlight.length > 0 ? species.sunlight.join(", ") : "N/A"
        } <br>
...
function displayHardinessResults(results) {
  ...
  results.forEach((species, index) => {
    ...
    accordionItem.innerHTML = `
      ...
      index === 0 ? "show" : ""
    }" aria-labelledby="${headerId}" data-bs-parent="#plantAccordion">
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            ${
                              ...
                              species.default_image.medium_url
                                ? `<img src="${species.default_image.medium_url}" alt="${species.common_name}" class="img-fluid rounded mb-3">`
                                : '<img src="https://via.placeholder.com/150" alt="No image available" class="img-fluid rounded mb-3">'
                            }
                        </div>
                        <div class="col-md-8">
                            <ul class="list-group">
                                <li class="list-group-item"><strong>Scientific Name:</strong> <em>${
                                  species.scientific_name.length > 0
                                    ? species.scientific_name.join(", ")
                                    : "N/A"
                                }</em></li>
                                <li class="list-group-item"><strong>Other Names:</strong> ${
                                  species.other_name.length > 0
                                    ? species.other_name.join(", ")
                                    : "N/A"
                                }</li>
                                <li class="list-group-item"><strong>Cycle:</strong> ${
                                  species.cycle || "N/A"
                                }</li>
                                <li class="list-group-item"><strong>Sunlight:</strong> ${
                                  species.sunlight.length > 0
                                    ? species.sunlight.join(", ")
                                    : "N/A"
                                }</li>
                                <li class="list-group-item"><strong>Watering:</strong> ${
                                  species.watering || "N/A"
                                }</li>
                            </ul>
                        </div>
                    </div>
...
function displayFavorites() {
  ...
  favorites.forEach((species, index) => {
    ...
    favoriteItem.innerHTML = `
            <div class="row g-0">
                <div class="col-md-4">
                    ${
                      species.default_image && species.default_image.medium_url
                        ? `<img src="${species.default_image.medium_url}" alt="${species.common_name}" class="img-fluid rounded-start">`
                        : '<img src="https://via.placeholder.com/150" alt="No image available" class="img-fluid rounded-start">'
                    }
                </div>
                <div class="col-md-8">
                    <div class="card-body">
    ...
    document
      .getElementById(`removeFavorite${index}`)
      .addEventListener("change", (event) => {
        if (!event.target.checked) {
          removeFromFavorites(species);
        }
...
function showHardinessErrorModal() {
  const hardinessErrorModal = new bootstrap.Modal(
    document.getElementById("hardinessErrorModal")
  );
